...
void system_t::config_proc()
{
    ...
    // Compute
    {
        ...
        ESP_REPORT_TIME(begin_time, "BEGIN - 2048");
        ...
        ESP_REPORT_TIME(end_time, "END - 2048");
...
void system_t::load_memory()
{
    ...
    out_words_adj = board;
#else
    in_words_adj = round_up(row * board, DMA_WORD_PER_BEAT);
    out_words_adj = round_up(board, DMA_WORD_PER_BEAT);
#endif

    in_size = in_words_adj * (board);
    out_size = out_words_adj * (board);

    in = new int64_t[in_size];
    for (int i = 0; i < board; i++)
        for (int j = 0; j < row * board; j++)
