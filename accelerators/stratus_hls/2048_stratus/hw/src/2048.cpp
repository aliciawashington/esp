...
#include "2048_functions.hpp"

// Processes

void 2048::load_input()
{
    ...
    // Load
    {
        ...
        // Batching
        for (uint16_t b = 0; b < board; b++)
        {
            wait();
#if (DMA_WORD_PER_BEAT == 0)
            uint32_t length = row * board;
#else
            uint32_t length = round_up(row * board, DMA_WORD_PER_BEAT);
#endif
            // Chunking
...
void 2048::store_output()
{
    ...
    // Store
    {
        ...
        bool ping = true;
#if (DMA_WORD_PER_BEAT == 0)
        uint32_t store_offset = (row * board) * board;
#else
        uint32_t store_offset = round_up(row * board, DMA_WORD_PER_BEAT) * board;
#endif
        ...
        for (uint16_t b = 0; b < board; b++)
        {
            wait();
#if (DMA_WORD_PER_BEAT == 0)
            uint32_t length = board;
#else
            uint32_t length = round_up(board, DMA_WORD_PER_BEAT);
#endif
            // Chunking
...
void 2048::compute_kernel()
{
    ...
    bool ping = true;
    {
        for (uint16_t b = 0; b < board; b++)
        {
            uint32_t in_length = row * board;
            uint32_t out_length = board;
